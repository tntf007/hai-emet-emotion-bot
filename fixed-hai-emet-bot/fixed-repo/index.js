...
// Search cache: store last 10 results per user
const SEARCH_CACHE = new Map();

/**
 * Internet web search (real search engine results).
 *
 * Providers (auto-select):
 *  1) Brave Search API (recommended): set BRAVE_SEARCH_API_KEY
 *     Docs: https://api.search.brave.com/
 *  2) SerpAPI (Google results via SerpAPI): set SERPAPI_KEY
...
async function webSearch(query, limit = 10) {
  ...
  if (braveKey) {
    const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(q)}&count=${encodeURIComponent(Math.min(limit, 10))}`;
    const res = await fetch(url, {
      headers: {
  ...
  // 2) SerpAPI (Google via SerpAPI)
  const serpKey = process.env.SERPAPI_KEY;
  if (serpKey) {
    const url = `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(q)}&num=${encodeURIComponent(Math.min(limit, 10))}&api_key=${encodeURIComponent(serpKey)}`;
    const res = await fetch(url);
  ...
  throw new Error(
    'לא הוגדר מנוע חיפוש אינטרנט. הוסף משתנה סביבה: BRAVE_SEARCH_API_KEY (מומלץ) או SERPAPI_KEY.'
  );
}
...
function getEngineMenu(type) {
  ...
  const buttons = engines.map(eng => {
    const costText = eng.free ? 'חינם' : `$${eng.cost}`;
...
bot.on('callback_query', async (q) => {
  ...
  if (d === 'menu_search') {
    ...
    // enter search mode
    userState.set(uid, { mode: 'search', step: 'query' });

    await bot.editMessageText(
      `🔎 **חיפוש (אינטרנט)**\n\n` +
      `כתוב מילה/ביטוי לחיפוש. אציג 10 תוצאות.\n\n` +
      `דוגמה: \`בינה מלאכותית\``,
      {
        chat_id: cid,
        message_id: mid,
...
bot.on("message", async (msg) => {
  ...
  if (state && state.mode === 'search') {
    ...
    if (state.step === 'query') {
      try {
        const results = await webSearch(text, 10);
        if (!results.length) {
          await bot.sendMessage(cid, `❌ לא נמצאו תוצאות עבור: ${text}`);
          return;
        }

        SEARCH_CACHE.set(uid, { query: text, results, ts: Date.now() });
        userState.set(uid, { mode: 'search', step: 'pick' });
...
bot.on("message", async (msg) => {
  ...
  if (state.step === 'type') {
    ...
    if (isNaN(cost) || cost < 0) {
      await bot.sendMessage(cid, `❌ עלות לא חוקית. שלח מספר (0 לחינם)`);
      return;
    }
    ...
    if (result.ok) {
      ...
      await bot.sendMessage(cid,
        ...
        `עלות: ${result.engine.free ? 'חינם' : '$' + result.engine.cost}\n\n` +
