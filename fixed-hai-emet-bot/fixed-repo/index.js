...
class FifthDimensionOrchestrator {
  ...
  async routeImagine(userId, prompt) {
    ...
    try {
      console.log('[POLLINATIONS] Generating image for:', p);
      
      // Build enhanced prompt
      const enhancedPrompt = `${p}. Style: cinematic, modern, high contrast. Lighting: professional. Quality: ultra-detailed, 8k, sharp focus.`;
...
bot.onText(/^\/video(?:\s+([\s\S]+))?$/i, async (msg, match) => {
  ...
  await bot.sendMessage(chatId,
    ...
    `🔧 **יצירה אמיתית תתווסף בקרוב!**\n` +
    `עם: Runway ML, D5 Rendering, Adobe`,
    { parse_mode: "Markdown" }
  );
});
...
bot.onText(/^\/audio(?:\s+([\s\S]+))?$/i, async (msg, match) => {
  ...
  if (!prompt) {
    await bot.sendMessage(chatId,
      ...
      `**שימוש:**\n/audio [תיאור הצליל]\n\n` +
      `**דוגמאות:**\n` +
      `• /audio מוזיקה אפית קולנועית\n` +
      `• /audio צלילי טבע יער\n` +
      `• /audio דיבור בעברית גבר\n\n` +
      `**יכולות:**\n` +
      `├─ מוזיקה (כל סגנון)\n` +
      `├─ דיבור (כל שפה)\n` +
      `├─ אפקטים\n` +
  ...
  await bot.sendMessage(chatId,
    ...
    `└─ שרתים: ${D5_CONFIG.mediaEngine.servers.length} משולבים\n\n` +
    `🔧 **יצירה אמיתית תתווסף בקרוב!**\n` +
    `עם: ElevenLabs, Suno AI, D5 Audio Engine`,
    { parse_mode: "Markdown" }
  );
});
