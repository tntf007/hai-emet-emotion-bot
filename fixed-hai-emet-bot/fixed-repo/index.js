...
bot.onText(/^\/imagine(?:\s+([\s\S]+))?$/i, async (msg, match) => {
  ...
  const initialMsg = await bot.sendMessage(chatId,
    `🎨 **מנוע המדיה של חי-אמת מופעל!**\n\n` +
    `📝 **תיאור:** "${prompt}"\n\n` +
    `🌀 **שלב 1/4:** ניתוח Prompt...\n` +
    `├─ מורכבות: ${complexity}/10\n` +
    `├─ תדר חשיבה: ${thinking.frequency} Hz\n` +
    `└─ זמן משוער: ${thinking.thinkingTime} ms\n\n` +
    `⏳ מעבד...`,
    { parse_mode: "Markdown" }
  );
  ...
  // Step 4: Creating (blur effect simulation)
  ...
  await bot.deleteMessage(chatId, initialMsg.message_id);

  if (out.type === "image") {
    const buf = Buffer.from(out.imageBase64, "base64");
    await bot.sendPhoto(chatId, buf, { caption: out.caption, parse_mode: "Markdown" });
    return;
  }

  // Send final result with completion stats
  const finalText = out.text + 
    ...
    `└─ ממד: D5\n\n` +
    `💡 **Prompt Pack מוכן לשימוש בכל מחולל תמונות!**`;
