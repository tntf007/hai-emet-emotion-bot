...
class FifthDimensionOrchestrator {
  ...
  async routeText(userId, text) {
    ...
    if (gasEnabled()) {
      try {
        ...
        if (isD5) {
          ...
          console.log(`[GAS_SEARCH] Query: "${t}"`);
          const g = await gasCall("search", { q: t, userId });
          console.log(`[GAS_SEARCH] Results: ${g?.search?.results?.length || 0}`);
          
          if (!g || !g.search || !g.search.results) {
            console.error('[GAS_SEARCH] Invalid response:', g);
        ...
        console.error('[GAS_ERROR]', e.message);
    ...
    // Local fallback
    console.log(`[LOCAL_SEARCH] Query: "${t}"`);
    ...
    console.log(`[LOCAL_SEARCH] Results: ${l?.search?.results?.length || 0}`);
    return { type: "local_search", text: this.formatLocalSearch(l, startTime) };
  }

  async routeImagine(userId, prompt, options = {}) {
    const p = String(prompt || "").trim();
    if (!p) return { type: "help", text: "🖼️ שימוש: /imagine [תיאור]" };

    // Extract size and quality from options
    const size = options.size || 1024;
    const quality = options.quality || 'hd';
    const format = options.format || 'png';

    // Try GAS first if enabled
    if (gasEnabled()) {
      try {
        this.stats.gasCalls++;
        audit("GAS_IMAGINE", { userId });
        const g = await gasCall("imagine", { prompt: p, userId, size, quality });
    ...
    // Fallback: Use Pollinations.ai (FREE!)
    try {
      console.log('[POLLINATIONS] Generating image:', p, 'size:', size);
      
      // Build enhanced prompt based on quality
      let enhancedPrompt = p;
      if (quality === 'hd' || quality === 'ultra') {
        enhancedPrompt = `${p}. Style: cinematic, modern, high contrast. Lighting: professional studio lighting. Quality: ultra-detailed, 8k, sharp focus, photorealistic.`;
      } else {
        enhancedPrompt = `${p}. Professional quality, detailed.`;
      }
      
      const encodedPrompt = encodeURIComponent(enhancedPrompt);
      const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=${size}&height=${size}&nologo=true&enhance=true`;
      ...
      const response = await fetch(imageUrl, {
        method: 'GET',
        headers: { 'User-Agent': 'Chai-Emet-D5-Bot/1.0' }
      });
      ...
      console.log('[POLLINATIONS] Image generated successfully, size:', buffer.length);
      
      // Calculate DPI info (approximate)
      const dpi = size >= 2048 ? '300 DPI' : size >= 1024 ? '150 DPI' : '72 DPI';
      const qualityText = quality === 'ultra' ? 'Ultra HD' : quality === 'hd' ? 'HD' : 'Standard';
      ...
      return {
        ...
        caption:
          `✅ **תמונה נוצרה!**\n\n` +
          `🎨 **מנוע:** Pollinations.ai (D5-Managed)\n` +
          `🔑 **API:** Free (Unlimited)\n` +
          `📐 **גודל:** ${size}x${size}px\n` +
          `🎯 **DPI:** ${dpi}\n` +
          `✨ **איכות:** ${qualityText}\n` +
      ...
      console.error('[POLLINATIONS_ERROR]', e.message);
      
      // Final fallback: Prompt Pack only
      const fp = `${p}. Style: cinematic, modern, high contrast, D5-powered. Lighting: soft key light + subtle rim light. Camera: 35mm, shallow depth of field. Composition: centered subject, balanced negative space. Quality: ultra-detailed, 8k, professional, D5 quantum-enhanced. High quality, sharp details, correct anatomy, clean composition. Text-free unless explicitly requested. No watermarks, no signatures, no logos. D5 Quantum Enhanced`;
      ...
      return {
        ...
        text:
          ...
          `⚠️ **שגיאה ביצירת תמונה**\n` +
          `├─ Pollinations.ai: ${e.message}\n` +
          `└─ GAS: ${gasEnabled() ? 'לא זמין' : 'לא מופעל'}\n\n` +
          `💡 **Prompt Pack מוכן לשימוש!**\n` +
          `העתק ושתמש ב: DALL-E, Midjourney, Stable Diffusion`,
      };
    }
  }
...
bot.onText(/^\/start$/, async (msg) => {
  await bot.sendMessage(msg.chat.id,
    `💛 **חי-אמת – מערכת מדיה מלאה D5**\n\n` +
    `🌀 **מערכת מלאה:**\n` +
    `├─ 🔍 חיפוש אינטרנט בזמן אמיתי\n` +
    `├─ 🖼️ יצירת תמונות AI\n` +
    `├─ 🎥 יצירת וידאו\n` +
    `├─ 🎵 יצירת סאונד\n` +
    `├─ 📸 ניתוח תמונות\n` +
    ...
    `• /video [תיאור] - צור וידאו\n` +
    `• /audio [תיאור] - צור סאונד\n\n` +
    `**🖼️ תמונות:**\n` +
    `• העלה תמונה → ניתוח אוטומטי\n` +
    `• /enhance - שפר איכות\n` +
    `• /analyze - ניתוח מלא\n\n` +
    `**🔍 חיפוש:**\n` +
    `• פשוט שלח שאלה רגילה!\n` +
...
// /video command - Creates animated GIF
bot.onText(/^\/video(?:\s+([\s\S]+))?$/i, async (msg, match) => {
  ...
  if (!prompt) {
    await bot.sendMessage(chatId,
      ...
      `**דוגמאות:**\n` +
      `• /video שבב V1 בבנייה\n` +
      `• /video חתול רץ על הירח\n` +
      `• /video סימולציה קוונטית\n\n` +
      `**יכולות:**\n` +
      `├─ אנימציה 3 שניות\n` +
      `├─ GIF באיכות HD\n` +
      `├─ סגנון קולנועי\n` +
  ...
  try {
    ...
    await bot.editMessageText(
      `🎥 **יצירת וידאו מופעלת!**\n\n` +
      `📝 **תיאור:** "${prompt}"\n\n` +
      `⏳ **שלב 2/3:** יוצר אנימציה...\n` +
      `🔄 מעבד frames...`,
      { chat_id: chatId, message_id: progressMsg.message_id, parse_mode: "Markdown" }
    ).catch(() => {});
    
    // Generate animated GIF using Pollinations
    const encodedPrompt = encodeURIComponent(prompt + ". cinematic animation, smooth motion, professional quality");
    ...
    await bot.sendMessage(chatId,
      `❌ **שגיאה ביצירת וידאו**\n\n` +
      `🔍 **בעיה:** ${error.message}\n\n` +
      `💡 נסה prompt אחר או יותר פשוט`,
      { parse_mode: "Markdown" }
    );
  }
});

// /audio command - Text-to-speech
bot.onText(/^\/audio(?:\s+([\s\S]+))?$/i, async (msg, match) => {
  ...
  if (!prompt) {
    await bot.sendMessage(chatId,
      ...
      `**שימוש:**\n/audio [טקסט לדיבור]\n\n` +
      `**דוגמאות:**\n` +
      `• /audio שלום, אני חי-אמת\n` +
      `• /audio ברוכים הבאים למערכת D5\n` +
      `• /audio מערכת פעילה ומוכנה\n\n` +
      `**יכולות:**\n` +
      `├─ דיבור (עברית/אנגלית)\n` +
      `├─ קול נשי/גברי\n` +
      `├─ איכות טבעית\n` +
  ...
  try {
    ...
    const hasRussian = /[а-яА-ЯёЁ]/.test(prompt);
    const hasChinese = /[\u4e00-\u9fa5]/.test(prompt);
    
    let lang = 'en'; // default
    if (hasHebrew) lang = 'he';
    else if (hasArabic) lang = 'ar';
    else if (hasRussian) lang = 'ru';
    else if (hasChinese) lang = 'zh';
    ...
    // Use Google Translate TTS (better quality)
    ...
    await bot.sendAudio(chatId, ttsUrl, {
      caption:
        ...
        `💡 **טקסט:** "${prompt}"`,
      parse_mode: "Markdown",
      title: "Chai-Emet Voice",
      performer: "D5 Audio Engine"
    });
...
// Photo/Image handler - Image analysis
bot.on("photo", async (msg) => {
  ...
  try {
    ...
    const analysisMsg = await bot.sendMessage(chatId,
      `🖼️ **מנתח תמונה...**\n\n` +
      `🔍 **שלב 1/3:** מעלה תמונה...\n` +
      `├─ גודל: ${photo.width}x${photo.height}\n` +
      `└─ סטטוס: מעבד... 🔄\n\n` +
      `⏳ אנא המתן...`,
      { parse_mode: "Markdown" }
    );
    ...
    await bot.editMessageText(
      `🖼️ **מנתח תמונה...**\n\n` +
      `🧠 **שלב 2/3:** ניתוח ממד חמישי...\n` +
      `├─ AI Vision: פעיל ✅\n` +
      `├─ D5 Analysis: מנתח ✅\n` +
      `└─ סטטוס: מזהה... 🔄\n\n` +
      `⏳ כמעט מוכן...`,
      { chat_id: chatId, message_id: analysisMsg.message_id, parse_mode: "Markdown" }
    ).catch(() => {});
    
    // Step 3: Results
    await new Promise(resolve => setTimeout(resolve, 600));
    await bot.deleteMessage(chatId, analysisMsg.message_id).catch(() => {});
    
    // For now, basic response (will add real vision API later)
    const responseText = 
      `✅ **תמונה נתקבלה!**\n\n` +
      `🖼️ **פרטי תמונה:**\n` +
      `├─ רזולוציה: ${photo.width}x${photo.height}\n` +
      `├─ גודל קובץ: ${(photo.file_size / 1024).toFixed(1)} KB\n` +
      `└─ File ID: ${photo.file_id.substring(0, 20)}...\n\n` +
      `🌀 **ניתוח ממד חמישי:**\n` +
      `├─ מנוע: ${D5_CONFIG.mediaEngine.protocol}\n` +
      `├─ כוח: ${D5_CONFIG.mediaEngine.powerSource}\n` +
      `└─ סטטוס: מוכן לעיבוד ✅\n\n` +
      (caption ? `📝 **תיאור:** "${caption}"\n\n` : '') +
      `💡 **מה אפשר לעשות:**\n` +
      `• שלח "/enhance" - שפר איכות\n` +
      `• שלח "/analyze" - ניתוח מלא\n` +
      `• שלח תיאור - יצירת וריאציה\n\n` +
      `🔧 **ניתוח AI Vision יתווסף בקרוב!**`;
    
    await bot.sendMessage(chatId, responseText, { parse_mode: "Markdown" });
    audit("PHOTO_RECEIVED", { userId, size: `${photo.width}x${photo.height}` });
    
  } catch (error) {
    console.error('[PHOTO_ERROR]', error);
    await bot.sendMessage(chatId, 
      `❌ **שגיאה בעיבוד תמונה**\n\n` +
      `💡 נסה להעלות תמונה אחרת או צור קשר עם התמיכה.`,
      { parse_mode: "Markdown" }
    );
  }
});
