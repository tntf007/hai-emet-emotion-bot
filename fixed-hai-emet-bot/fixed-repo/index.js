...
// Optional: Tor (Onion) routing via SOCKS5 proxy (e.g., Tor)
...
// ==========================================
// SEARCH (Best Web Search + Optional Tor)
// ==========================================
// Stores last results per user (max 10)
const SEARCH_CACHE = new Map();

function getTorAgent() {
  const proxy = process.env.TOR_SOCKS_PROXY || process.env.SOCKS_PROXY || '';
  if (!proxy) return null;
  if (!SocksProxyAgent) return null;
  try { return new SocksProxyAgent(proxy); } catch (e) { return null; }
}

async function fetchMaybeTor(url, options = {}, useTor = false) {
  if (!useTor) return fetch(url, options);
  const agent = getTorAgent();
  if (!agent) return fetch(url, options); // fallback to normal fetch
  return fetch(url, { ...options, agent });
}

/**
 * Best-effort Web Search (no scraping).
 * Priority (auto):
 *  1) Brave Search API (recommended) -> BRAVE_SEARCH_API_KEY
 *  2) Bing Web Search (Azure)        -> BING_SEARCH_API_KEY (+optional BING_SEARCH_ENDPOINT)
 *  3) Google Custom Search (CSE)     -> GOOGLE_CSE_API_KEY + GOOGLE_CSE_CX
 *  4) SerpAPI (Google via SerpAPI)   -> SERPAPI_KEY
 *
 * Optional Tor routing:
 *  - set TOR_SOCKS_PROXY=socks5h://127.0.0.1:9050  (or your proxy)
 *  - set SEARCH_USE_TOR=1 (default off)
 *
 * Returns: [{ title, snippet, url, source }]
 */
async function webSearch(query, limit = 10, opts = {}) {
  const q = String(query || '').trim();
  if (!q) return [];
  const n = Math.max(1, Math.min(10, Number(limit) || 10));
  const useTor = !!opts.useTor;

  // 1) Brave
  const braveKey = process.env.BRAVE_SEARCH_API_KEY;
  if (braveKey) {
    const url = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(q)}&count=${encodeURIComponent(n)}`;
    const res = await fetchMaybeTor(url, {
  ...
  // 2) Bing (Azure Cognitive Services)
  ...
  if (bingKey) {
    ...
    const url = `${endpoint}?q=${encodeURIComponent(q)}&count=${encodeURIComponent(n)}&mkt=he-IL`;
    const res = await fetchMaybeTor(url, {
      headers: { "Ocp-Apim-Subscription-Key": bingKey, "Accept": "application/json" }
    }, useTor);
    if (!res.ok) throw new Error(`Bing HTTP ${res.status}`);
    const data = await res.json();
    return (data?.webPages?.value || []).slice(0, n).map(r => ({
  ...
  if (gKey && gCx) {
    const url = `https://www.googleapis.com/customsearch/v1?key=${encodeURIComponent(gKey)}&cx=${encodeURIComponent(gCx)}&q=${encodeURIComponent(q)}&num=${encodeURIComponent(n)}`;
    const res = await fetchMaybeTor(url, { headers: { "Accept": "application/json" } }, useTor);
    if (!res.ok) throw new Error(`Google CSE HTTP ${res.status}`);
    const data = await res.json();
    return (data?.items || []).slice(0, n).map(r => ({
      title: r?.title || 'Untitled',
      snippet: r?.snippet || '',
      url: r?.link || '',
      source: 'google_cse'
  ...
  if (serpKey) {
    const url = `https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(q)}&num=${encodeURIComponent(n)}&api_key=${encodeURIComponent(serpKey)}`;
    const res = await fetchMaybeTor(url, { headers: { "Accept": "application/json" } }, useTor);
    if (!res.ok) throw new Error(`SerpAPI HTTP ${res.status}`);
    const data = await res.json();
    return (data?.organic_results || []).slice(0, n).map(r => ({
      title: r?.title || 'Untitled',
      snippet: r?.snippet || '',
      url: r?.link || '',
      source: 'serpapi'
  ...
  throw new Error('לא הוגדר מנוע חיפוש. הגדר BRAVE_SEARCH_API_KEY (מומלץ) או BING_SEARCH_API_KEY או GOOGLE_CSE_API_KEY+GOOGLE_CSE_CX או SERPAPI_KEY.');
}
...
[
  ...
  { text: '🧅 חיפוש Tor', callback_data: 'menu_search_tor' }
],
[
...
if (d === 'menu_search' || d === 'menu_search_tor') {
  await bot.answerCallbackQuery(q.id);

  const useTor = (d === 'menu_search_tor');

  // enter search mode
  userState.set(uid, { mode: 'search', step: 'query', useTor });

  const title = useTor ? '🧅 חיפוש Tor (אינטרנט)' : '🔎 חיפוש אינטרנט';
  const note = useTor
    ? 'החיפוש יעבור דרך SOCKS/Tor אם הוגדר TOR_SOCKS_PROXY. אם לא הוגדר, זה יעבוד כחיפוש רגיל.'
    : 'חיפוש תוצאות אינטרנט כמו מנוע חיפוש.';

  await bot.editMessageText(
    `${title}\n\n${note}\n\nכתוב מילה/ביטוי. אציג עד 10 תוצאות ואז תבחר מספר 1-10.`,
    {
      chat_id: cid,
      message_id: mid,
      parse_mode: "Markdown",
...
if (d.startsWith('search_pick_')) {
  await bot.answerCallbackQuery(q.id);

  const idx = parseInt(d.replace('search_pick_', ''), 10) - 1;
  const cached = SEARCH_CACHE.get(uid);

  if (!cached || !Array.isArray(cached.results) || !cached.results[idx]) {
    await bot.sendMessage(cid, `⚠️ אין תוצאות זמינות. נסה חיפוש מחדש.`);
    return;
  }

  const r = cached.results[idx];
...
// SEARCH MODE: user typed a query OR picked a result number
if (state && state.mode === 'search') {
  // Step 1: user sends query
  if (state.step === 'query') {
    try {
      const results = await webSearch(text, 10, { useTor: !!state.useTor });
      if (!results.length) {
        await bot.sendMessage(cid, `❌ לא נמצאו תוצאות עבור: ${text}`);
        return;
      }

      SEARCH_CACHE.set(uid, { query: text, results, ts: Date.now(), useTor: !!state.useTor });
      userState.set(uid, { mode: 'search', step: 'pick', useTor: !!state.useTor });

      // Build numbered list
      let out = `🔎 **תוצאות לחיפוש:** ${text}\n\n`;
      results.forEach((r, i) => {
        out += `${i + 1}. ${r.title}\n`;
      });
      out += `\nבחר מספר (1-10) או לחץ על כפתור.`;

      // Build number keyboard (1..10)
      const keys = [];
      for (let i = 1; i <= Math.min(10, results.length); i += 2) {
        const row = [{ text: String(i), callback_data: `search_pick_${i}` }];
        if (i + 1 <= results.length) row.push({ text: String(i + 1), callback_data: `search_pick_${i + 1}` });
        keys.push(row);
      }
      keys.push([{ text: '🔙 תפריט', callback_data: 'menu_main' }]);

      await bot.sendMessage(cid, out, { parse_mode: "Markdown", reply_markup: { inline_keyboard: keys } });
      return;
    } catch (err) {
      await bot.sendMessage(cid, `❌ שגיאת חיפוש: ${err.message}`);
      return;
    }
  }

  // Step 2: user picks 1..10
  if (state.step === 'pick') {
    const n = parseInt(text, 10);
    if (!Number.isFinite(n) || n < 1 || n > 10) {
      await bot.sendMessage(cid, `⚠️ שלח מספר בין 1 ל-10 כדי לבחור תוצאה.`);
      return;
    }
    const cached = SEARCH_CACHE.get(uid);
    const r = cached?.results?.[n - 1];
    if (!r) {
      await bot.sendMessage(cid, `⚠️ תוצאה לא קיימת. נסה חיפוש מחדש.`);
      userState.set(uid, { mode: 'search', step: 'query', useTor: !!state.useTor });
      return;
    }
    await bot.sendMessage(
      cid,
