// ==========================================
// CHAI-EMET v18.0 ULTIMATE
// ✅ בחירת מידות
// ✅ מידות דרך prompt
// ✅ מנוע ליצירת prompts (AI)
// ✅ הכל עובד!
// ==========================================
...
app.get("/", (req, res) => res.json({ status: 'online', version: '18.0-ULTIMATE' }));
...
async function generateAudio(text, engineKey) {
  ...
  try {
    const url = `https://api.responsivevoice.org/v1/text:synthesize?text=${encodeURIComponent(text)}&lang=he&engine=g1&name=Hebrew%20Female&rate=0.5&pitch=1&volume=1&key=free`;
    ...
    if (gen.ok) {
      ...
      if (type === 'image' || type === 'video') {
        await bot.sendPhoto(cid, gen.data, {
          caption: `✅ ${engine.name}${type === 'image' ? `\n📐 ${promptData.size.name}` : ''}\n⏱️ ${(gen.time/1000).toFixed(1)}s\n💰 ${bal} HET | 🌀 ${d5} D5`
        });
      } else {
...
console.log('🌀 v18.0 ULTIMATE: ONLINE');
